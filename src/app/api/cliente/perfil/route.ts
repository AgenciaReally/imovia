...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log('Cookies disponíveis:', req.cookies.getAll());
    ...
    if (!session || !session.user || !session.user.email) {
      console.log('Falha na autenticação - sessão inválida');
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (dadosPerfil.dataNascimento) dadosAtualizacao.dataNascimento = new Date(dadosPerfil.dataNascimento);
    if (dadosPerfil.endereco) dadosAtualizacao.endereco = dadosPerfil.endereco;
    if (dadosPerfil.cidade) dadosAtualizacao.cidade = dadosPerfil.cidade;
    if (dadosPerfil.estado) dadosAtualizacao.estado = dadosPerfil.estado;
    if (dadosPerfil.cep) dadosAtualizacao.cep = dadosPerfil.cep;
