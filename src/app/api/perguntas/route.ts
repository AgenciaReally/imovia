...
export async function POST(request: NextRequest) {
  try {
    ...
    const dadosParaInserir = {
      ...
      fluxo: data.fluxo || 'PRINCIPAL', // Valor padrão se não fornecido
      step: data.step || 1, // Adicionar campo step
      ...
      mascaraValor: data.mascaraValor !== undefined ? data.mascaraValor : false,
      insightIA: data.insightIA !== undefined ? data.insightIA : false,
      mascaraCidade: data.mascaraCidade !== undefined ? data.mascaraCidade : false,
      mascaraBairro: data.mascaraBairro !== undefined ? data.mascaraBairro : false,
      opcoes: data.opcoes || null,
      condicional: data.condicional || null,
      ordem: data.ordem || ordem // Usar ordem fornecida ou calculada
    }
