      ...
      <Card className="overflow-hidden h-full flex flex-col border-0 shadow-md hover:shadow-lg transition-all duration-200">
        ...
        <CardHeader className="p-4 pb-2">
          ...
          <div className="flex items-center text-sm text-muted-foreground">
            ...
            <span className="line-clamp-1">{imovel.bairro ? `${imovel.bairro}, ${imovel.cidade}` : imovel.cidade || 'Localização disponível'}</span>
          </div>
        </CardHeader>
        ...
        <CardFooter className="p-4 pt-2 flex gap-2">
          <Button 
            variant="default" 
            className="w-full"
            onClick={() => router.push(`/painel/cliente/imovel/${imovel.id}`)}
          >
...
export default function ImoveisClientePage() {
  ...
  const carregarDados = async () => {
    ...
    setLoading(true)
    try {
      // Carregar imóveis recomendados da API
      const recomendadosResponse = await fetch('/api/cliente/imoveis?tipo=MATCH');
      let imoveisRecomendadosAPI: Imovel[] = [];
      
      if (recomendadosResponse.ok) {
        const { success, data } = await recomendadosResponse.json();
        if (success && Array.isArray(data)) {
          imoveisRecomendadosAPI = data;
          console.log('✅ Imóveis recomendados carregados da API:', imoveisRecomendadosAPI.length);
        }
      } else {
        console.error('Erro ao carregar recomendados da API');
        
        // Fallback: tentar localStorage se a API falhar
        try {
          ...
          if (savedImoveis) {
            ...
            if (Array.isArray(parsedImoveis) && parsedImoveis.length > 0) {
              imoveisRecomendadosAPI = parsedImoveis.map(imovel => ({
                ...imovel,
                matchPercentage: imovel.matchPercentage || 90 + (Math.floor(Math.random() * 10)),
                favorito: false
              }));
              console.log('📦 Fallback: Imóveis carregados do localStorage:', imoveisRecomendadosAPI.length);
            }
          }
        } catch (storageError) {
          console.error('Erro no fallback do localStorage:', storageError);
        }
      }
      
      // Salvar imóveis recomendados no state
      setImoveisRecomendados(imoveisRecomendadosAPI);
      
      // Carregar todos os imóveis salvos (incluindo favoritos, visitados, etc.)
      const todosResponse = await fetch('/api/cliente/imoveis?tipo=ALL');
      let todosImoveis: Imovel[] = [];
      
      if (todosResponse.ok) {
        const { success, data } = await todosResponse.json();
        if (success && Array.isArray(data)) {
          todosImoveis = data;
          console.log('✅ Todos os imóveis carregados da API:', todosImoveis.length);
        }
      } else {
        // Fallback: usar dados mockados se a API falhar
        todosImoveis = [...Array(12)].map((_, i) => ({
          id: `imovel-${i}`,
