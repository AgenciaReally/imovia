...
// Token expira em 12 horas por padrão
const DEFAULT_TOKEN_EXPIRATION = 12 * 60 * 60; // em segundos
 ...
 */
export async function createSessionToken(userId: string, email: string, role: UserRole, tokenExpiration?: number) {
  try {
    const expiration = tokenExpiration || DEFAULT_TOKEN_EXPIRATION;
