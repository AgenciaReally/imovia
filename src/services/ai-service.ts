  ...
  private async queryDatabase(message: string): Promise<string | null> {
    try {
      ...
      if (lowerMessage.includes('imóveis') || lowerMessage.includes('imoveis') || 
        ...
        const imoveisInfo = await AIDatabaseService.buscarImoveis({})
        
        if (imoveisInfo && imoveisInfo.success && imoveisInfo.imoveis) {
          // Formatar resposta de acordo com o tipo de consulta
          if (lowerMessage.includes('quantos')) {
            return `Atualmente há ${imoveisInfo.imoveis.length} imóveis no banco de dados.`
          }
          
          if (lowerMessage.includes('listar') || lowerMessage.includes('mostrar')) {
            const imoveisList = imoveisInfo.imoveis
              .map((i: any) => `${i.titulo} (ID: ${i.id}, Preço: R$${i.preco})`)
              .join('\n- ')
          ...
          if (lowerMessage.includes('detalhe') || lowerMessage.includes('encontrar')) {
            ...
            if (idMatch && idMatch[1]) {
              const id = idMatch[1]
              const imovelEncontrado = imoveisInfo.imoveis.find((i: any) => i.id === id)
              ...
              if (imovelEncontrado) {
                return `Detalhes do imóvel ID ${id}:\n` +
                  ...
                  `Status: ${imovelEncontrado.status}`
              }
            }
            
            // Se não houver ID, retornar uma lista geral
            return `Encontrei ${imoveisInfo.imoveis.length} imóveis. Para ver detalhes de um imóvel específico, informe o ID.`
          }
          
          return `Encontrei informações sobre ${imoveisInfo.imoveis.length} imóveis no sistema.`
        }
      }
      ...
      if (lowerMessage.includes('construtoras') || lowerMessage.includes('construtora')) {
        ...
        if (construtorasInfo && construtorasInfo.success && construtorasInfo.construtoras) {
          if (lowerMessage.includes('quantas')) {
            return `Existem ${construtorasInfo.total} construtoras cadastradas na plataforma.`
          }
          
          const construtorasList = construtorasInfo.construtoras
            .map((c: any) => `${c.nome} (ID: ${c.id}, Imóveis: ${c.imoveis})`)  
            .join('\n- ')
      ...
      if (lowerMessage.includes('clientes') || lowerMessage.includes('usuários') || lowerMessage.includes('usuarios')) {
        const usersInfo = await AIDatabaseService.getClientes()
        
        if (usersInfo && usersInfo.success && usersInfo.clientes) {
          if (lowerMessage.includes('quantos')) {
            return `Existem ${usersInfo.total} clientes cadastrados na plataforma.`
          }
          
          const usersList = usersInfo.clientes
      ...
      // Busca de relatórios - método não implementado ainda
      if (lowerMessage.includes('relatórios') || lowerMessage.includes('relatorios')) {
        return `Funcionalidade de relatórios ainda não implementada no sistema de chat.`
      }
      ...
      if (lowerMessage.includes('perguntas') || lowerMessage.includes('formulário') || lowerMessage.includes('formulario')) {
        ...
        if (perguntasInfo && perguntasInfo.success && perguntasInfo.perguntas) {
          if (lowerMessage.includes('quantas')) {
            return `Existem ${perguntasInfo.total} perguntas cadastradas no sistema.`
          }
          
          if (lowerMessage.includes('categoria')) {
            // Calcular estatísticas de categoria a partir das perguntas
            const categorias = perguntasInfo.perguntas.reduce((acc: any, p: any) => {
              acc[p.categoria] = (acc[p.categoria] || 0) + 1
              return acc
            }, {})
            
            const categoriasList = Object.entries(categorias)
              .map(([categoria, count]) => `${categoria}: ${count} perguntas`)
              .join('\n- ')
              
            return `Perguntas por categoria:\n- ${categoriasList}`
          }
          
          return `Existem ${perguntasInfo.total} perguntas no sistema.`
        }
      }
